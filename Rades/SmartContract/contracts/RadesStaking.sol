...
contract RadesStaking is Ownable {
    ERC20 public stakeToken;
    uint256 public totalStakedAmount = 0;
    uint256 public totalUnStakedAmount = 0;
    uint256 private constant INVALID_INDEX = 999;

    uint256 public constant twoWeekPoolLimit = 20 * 10 ** 6;

    uint256 public constant oneMonthPoolLimit = 20 * 10 ** 6;
    ...
    function stake(uint256 _amount, uint _termOption) internal {
        require(
            _amount >= (minAmount * 10 ** stakeToken.decimals()),
            "Stake amount must be greater than 2000"
        );
    ...
    function unStake(uint index) public {
        ...
        require(
            (staker.releaseDate <= block.timestamp),
            "You can't unstake before release date"
        );
    ...
    function twoWeekStake(
        uint256 _amount
    ) public underTwoWeekPoolRemain(_amount) {
        stake(_amount, 14);
    }

    function oneMonthStake(
        uint256 _amount
    ) public underOneMonthPoolRemain(_amount) {
        stake(_amount, 30);
    }
    ...
    function getStakerInfoByTermOption(
        address _staker,
        uint _termOption,
        uint from,
        uint to
    ...
    function getStakerInfoByTermOptionAndRelease(
        ...
        for (uint i = 0; i < stakerInfo.length; i++) {
            if (
                stakerInfo[i].termOption == _termOption &&
                stakerInfo[i].isRelease == _isRelease
            ) {
                length++;
            }
        }
    ...
    function totalRewardDebtByAddress(
        address _address
    ) public view returns (uint _staked) {
        uint total = 0;
        StakerInfo[] storage staker = stakers[_address];
        for (uint256 i = 0; i < staker.length; i++) {
    ...
    {
        return (
            stakers[_staker][_index].amount,
            stakers[_staker][_index].releaseDate,
            stakers[_staker][_index].isRelease,
            stakers[_staker][_index].rewardDebt
        );
    }

    function getStakeInfoByIndex(
        uint _index
    )
        public
        view
    ...
    function totalStakerInfoByTermOption(
        address _staker,
        uint _termOption
    ...
    function totalStakerInfoByTermOptionAndRelease(
        address _staker,
        uint _termOption,
        bool _isRelease
    ) public view returns (uint) {
        ...
        for (uint256 i = 0; i < staker.length; i++) {
            if (
                staker[i].termOption == _termOption &&
                staker[i].isRelease == _isRelease
            ) {
                total++;
            }
        }
    ...
    function totalStakerInfoByRelease(
        address _staker,
        bool _isRelease
    ) public view returns (uint) {
        uint total = 0;
    ...
    modifier underOneMonthPoolRemain(uint256 _amount) {
        require(
            oneMonthPoolRemain() >= _amount,
            "One month pool limit reached"
        );
        _;
    }
}
