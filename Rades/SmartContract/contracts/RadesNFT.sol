...
contract RadesNFT is ERC1155 {
    ...
    modifier isCalledMarketplace() {
        require(_marketplace == msg.sender, "Invalid") ;
        _;
    }
    ...
    function transfer(uint256 _nftId, address _from, address _to, uint256 _amount) external isCalledMarketplace {
        _setApprovalForAll(_from, msg.sender, true);
        safeTransferFrom(_from, _to, _nftId, _amount, '');
    }

    function _balanceOf(address _owner, uint256 _nftId) external view returns(uint256) {
        return balanceOf(_owner, _nftId);
    }

    function setMarketplaceToNFT(address marketplace) external {
