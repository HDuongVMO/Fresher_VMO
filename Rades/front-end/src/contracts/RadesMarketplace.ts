...
import { RadesMarketplaceAbi } from "./utils/getAbis";
import { getRadesMarketplaceAddress } from "./utils/getAddress";

export default class RadesMarketplace extends ERC721 {
  constructor(provider?: ethers.providers.Web3Provider) {
    const rpcProvider = new ethers.providers.JsonRpcProvider(getRPC());
    super(provider || rpcProvider, getRadesMarketplaceAddress(), RadesMarketplaceAbi())
    if (!provider) {
