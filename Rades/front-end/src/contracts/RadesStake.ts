...
import { getRadesStakeAddress } from "./utils/getAddress";
import { getRPC } from "./utils/common";
import { getRadesStakeAbi } from "./utils/getAbis";
import { IStakerInfo } from "@/_types_";
import { getDays, isAfter, parseBalance } from "@/utils";
...
export default class RadesStake extends BaseInterface {
    constructor(provider?: ethers.providers.Web3Provider) {
        const rpcProvider = new ethers.providers.JsonRpcProvider(getRPC());
        super(provider ?? rpcProvider, getRadesStakeAddress(), getRadesStakeAbi());
        if (!provider) {
