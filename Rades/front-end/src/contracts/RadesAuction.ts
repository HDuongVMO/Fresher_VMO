...
import { getRadesAuctionAbi } from "./utils/getAbis";
import { getRadesAuctionAddress } from "./utils/getAddress";

export  default class AuctionContract extends BaseInterface {
  constructor(provider?: ethers.providers.Web3Provider) {
    const rpcProvider = new ethers.providers.JsonRpcProvider(getRPC());
    super(provider || rpcProvider, getRadesAuctionAddress(), getRadesAuctionAbi());
    if (!provider) {
